# Criar-template-de-newsletter-para-divulgacao-interna-de-novos-produtos
Atividade DIO LUPO

![Visual da Newsletter Interna](imagem_Newsletter.png)

---

📰 Newsletter Interna — Novidades de Produto (03 a 07 de março)
👋 Abertura
Nesta semana, nosso foco está em melhorias que aceleram o onboarding e aumentam a adoção dos produtos internos. As novidades foram pensadas para reduzir fricções e dar mais autonomia aos times.

---
🚀 Novidades da semana
⭐ Portal de Onboarding 2.0
O que é: nova área centralizada com trilhas por função e checklists automatizados
Por que isso importa: reduz dúvidas recorrentes e diminui tempo para produtividade inicial
Pra quem é: novos colaboradores e gestores responsáveis por onboarding
Como acessar/usar: intranet > Pessoas > Onboarding
Status: rollout (30% das áreas)
Links úteis: Portal de Onboarding | Documentação
📣 CTA interno: testem a trilha da sua área e enviem feedback no canal #onboarding-feedback

---
⭐ Catálogo de Produtos Internos
O que é: página única com todos os produtos internos, donos e guias rápidos
Por que isso importa: ajuda a encontrar ferramentas certas e evita retrabalho
Pra quem é: todos os times
Como acessar/usar: intranet > Produtos
Status: lançado
Links úteis: Catálogo de Produtos
📣 CTA interno: favoritar a página e sugerir produtos faltantes via formulário interno

---

⭐ Integração SSO no Painel de Vendas
O que é: login único (SSO – Single Sign-On) para acessar o painel sem múltiplas senhas
Por que isso importa: menos fricção e mais segurança
Pra quem é: time comercial e CS (Customer Success)
Como acessar/usar: acessar normalmente pelo link do painel; autenticação será automática
Status: beta
Links úteis: ⚠️ Informação pendente
📣 CTA interno: quem estiver no beta, reportar bugs no canal #painel-vendas

---
🧠 Resumo executivo
Portal de Onboarding 2.0 já em rollout parcial
Catálogo de Produtos Internos lançado para todos os times
Integração SSO em fase beta no Painel de Vendas
❓ FAQ rápido (opcional)
Quem já tem acesso ao Portal de Onboarding 2.0? → 30% das áreas estão incluídas no rollout inicial.
O Catálogo de Produtos será atualizado automaticamente? → Sim, os donos de cada produto são responsáveis por manter as informações atualizadas.

---

📬 Encerramento
Essas melhorias foram pensadas para facilitar o dia a dia e aumentar a eficiência dos times. Em caso de dúvidas, procure os canais internos de suporte ou fale diretamente com os responsáveis de cada produto.


